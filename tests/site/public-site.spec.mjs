...
const stable = JSON.parse(fs.readFileSync('CURRENT_STABLE.json', 'utf8'));
...
const sourceUrl = 'https://github.com/SupraCraft/Bridge';
const allowedExternalUrls = new Set([
  sourceUrl,
  stable.qualification,
  ...Object.values(stable.artifacts).map(item => item.download_url),
]);
const routes = [
  '/',
  '/use/',
...
test('primary Bridge user journeys stay on the friendly site', async ({ page }) => {
  await page.goto(routeUrl('/'));
  assertExpectedSite(page, 'homepage');

  await page.getByRole('link', { name: `Use Bridge ${stable.version}` }).click();
  ...
  assertExpectedSite(page, 'releases journey');
  await expect(page).toHaveURL(/\/releases\/$/);
  await page.getByRole('link', { name: `Bridge ${stable.version}` }).click();
  assertExpectedSite(page, 'release detail journey');
  await expect(page).toHaveURL(new RegExp(`/releases/${stable.version.replaceAll('.', '\\.')}\/$`));

  const primaryArtifact = page.getByRole('link', { name: /^Download/ }).first();
  await expect(primaryArtifact).toHaveAttribute('href', stable.artifacts.bridge.download_url);
  await expect(primaryArtifact).toHaveAttribute('target', '_blank');
  expect(stable.artifacts.bridge.download_url).toContain('/releases/download/');
});
